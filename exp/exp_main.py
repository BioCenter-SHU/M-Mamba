...
class Exp_Main(Exp_Basic):
    ...
    def create_multi_resolution_sequences(self, batch_x, resolutions):
        multi_res_sequences = {}
        batch_size, sequence_length, num_features = batch_x.shape
        for resolution in resolutions:
            res_sequence = batch_x[:, ::resolution, :]

            if res_sequence[:, -1, :].sum() != batch_x[:, -1, :].sum():
                last_values = batch_x[:, -1:, :]
                res_sequence = torch.cat((res_sequence, last_values), dim=1)

            num_segments = (sequence_length - 1) // resolution
            last_point_index = num_segments * resolution
            if last_point_index == sequence_length - 1:
                normalLength = num_segments + 1
            else:
                normalLength = num_segments + 2

            if res_sequence.shape[1] > normalLength:
                res_sequence = res_sequence[:, :normalLength, :]
            elif res_sequence.shape[1] < normalLength:
                last_values = batch_x[:, -1:, :]
                res_sequence = torch.cat((res_sequence, last_values.repeat(1, normalLength - res_sequence.shape[1], 1)),
                                         dim=1)

            multi_res_sequences[resolution] = res_sequence

        return multi_res_sequences

    def getResDict(self):
        x_dict = self.args.resolutions
        return x_dict
            ...
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_loader):
                ...
                if self.args.use_amp:
                    with torch.cuda.amp.autocast():
                        if 'M_Mamba' in self.args.model:
                            outputs = self.model(batch_x)

                else:
                    if 'M_Mamba' in self.args.model:
                        multi_res_sequences = self.create_multi_resolution_sequences(batch_x, self.getResDict())
                        outputs = self.model(multi_res_sequences)
